...
import PageTree from '../components/project/PageTree.tsx';
...
function ProjectDetail() {
  ...
  const { currentProject, pages, loading, error, fetchProject, fetchPages, deleteProject } = useProjectStore();
  ...
  const fetchHierarchy = useCallback(async () => {
    if (!id) return;
    try {
      const data = await apiGet<PageHierarchyNode[]>(`/projects/${id}/page-hierarchy`);
      setHierarchyNodes(data);
    } catch {
      // Fallback: if hierarchy endpoint fails, show empty tree
      setHierarchyNodes([]);
    }
  }, [id]);
  ...
  }, [id, fetchProject, fetchPages, fetchHierarchy]);
    ...
    <div className="project-detail">
      ...
      <div className="pages-section">
        ...
        {pages.length > 0 ? (
          <PageTree
            projectId={id || ''}
            nodes={hierarchyNodes.length > 0 ? hierarchyNodes : pages.map((p: any) => ({
              id: p.id,
              name: p.name,
              url: p.url || '',
              parentId: null,
